...
export async function PATCH(
    ...
    try {
        ...
        if (interaction.type !== INTERACTION_TYPE.NOTE_ADDED) {
            return NextResponse.json({ success: false, message: "Only notes can be edited" }, { status: 409 })
        }

        const titleChanged = incomingTitle !== undefined && incomingTitle !== (interaction.title ?? "")
